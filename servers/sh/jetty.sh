...
JENKINS_HOME=/opt/dev/jenkins/work
export JENKINS_HOME=/opt/dev/jenkins/work

JETTY_HOME=/opt/dev/jenkins
#JETTY_BASE=/opt/dev/jetty/demo-base
# 默认是/tmp（不够安全，因为系统服务可能会不定时删除/tmp目录）
TMPDIR=$JETTY_HOME/tmp
JETTY_RUN=$TMPDIR
JETTY_STATE=$TMPDIR/$NAME.state
JAVA_OPTIONS="-server -Xms256m -Xmx512m -XX:PermSize=256m -XX:MaxPermSize=256m -XX:-UseSplitVerifier -DJENKINS_HOME=$JENKINS_HOME"
JETTY_USER=root
...
case "$ACTION" in
  ...
  check|status)
    echo "Checking arguments to ${NAME}: "
    echo "START_INI      =  $START_INI"
    echo "START_D        =  $START_D"
    echo "JETTY_HOME     =  $JETTY_HOME"
    echo "JETTY_BASE     =  $JETTY_BASE"
